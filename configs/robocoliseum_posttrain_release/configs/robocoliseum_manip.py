"""GigaBrain-0.7 post-training config for manipulation tasks."""

import json
import os
from pathlib import Path

# Importing the package registers RoboColiseum custom datasets and transforms.
import robocoliseum_ext  # noqa: F401
from robocoliseum_ext.annotated_dataset import (  # noqa: F401
    AnnotatedTaskLeRobotDataset,
    ConcatenatedInstructionSegmentsLeRobotDataset,
...
training_tasks = ('clean_the_desktop',)
training_variant = os.environ.get('MANIP_TRAINING_VARIANT', 'first_step')
clean_desktop_dataset_classes = {
    'first_step': 'FirstInstructionSegmentLeRobotDataset',
    'concat_subinstructions': 'ConcatenatedInstructionSegmentsLeRobotDataset',
}
max_steps_by_variant = {
    'first_step': 5000,
    'concat_subinstructions': 80000,
}
if training_variant not in clean_desktop_dataset_classes:
    raise ValueError(
        'MANIP_TRAINING_VARIANT must be one of '
        f'{tuple(clean_desktop_dataset_classes)}, got {training_variant!r}'
    )

raw_gpu_ids = os.environ.get('MANIP_GPU_IDS', '0,1,2,3')
try:
    gpu_ids = [int(gpu_id) for gpu_id in raw_gpu_ids.split(',')]
except ValueError as error:
    raise ValueError(f'MANIP_GPU_IDS must contain integers, got {raw_gpu_ids!r}') from error
if len(gpu_ids) not in (4, 8) or len(set(gpu_ids)) != len(gpu_ids) or min(gpu_ids) < 0:
    raise ValueError(
        'MANIP_GPU_IDS must contain four or eight distinct non-negative GPU ids, '
        f'got {raw_gpu_ids!r}'
    )

run_name = f'robocoliseum_manip_{training_variant}'
max_steps = max_steps_by_variant[training_variant]
...
for data_path in data_paths:
    dataset_class_name = (
        clean_desktop_dataset_classes[training_variant]
        if Path(data_path).name == 'clean_the_desktop'
        else 'AnnotatedTaskLeRobotDataset'
    )
...
norm_stats_path = str(
    NORM_STATS_ROOT / f'robocoliseum_manip_{UNIFIED_ACTION_DIM}d.json'
)
...
config = dict(
    ...
    train=dict(
        ...
        checkpoint_keeps=list(range(5000, max_steps + 1, 5000)),
