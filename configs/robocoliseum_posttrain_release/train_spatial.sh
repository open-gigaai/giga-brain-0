...
: "${ROBOCOLISEUM_DATA_ROOT:?Set ROBOCOLISEUM_DATA_ROOT to the raw task_suite directory}"
...
PREPROCESS_SCRIPT=${EXPERIMENT_ROOT}/preprocess_spatial_g2a_17d.py
PROCESSED_SUITE_ROOT=${ROBOCOLISEUM_PROCESSED_DATA_ROOT}/spatial
CONFIG_RELATIVE_PATH=configs/robocoliseum_spatial.py
CONFIG_PATH=${EXPERIMENT_ROOT}/${CONFIG_RELATIVE_PATH}
NORM_STATS_PATH=${ROBOCOLISEUM_NORM_STATS_ROOT}/robocoliseum_spatial_17d.json
MODEL_DIR=${GIGABRAIN_PRETRAINED_CKPT}
MODEL_CONFIG=${MODEL_DIR}/config.json
PYTHON_BIN=${PYTHON_BIN:-python}
RUN_DIR=${ROBOCOLISEUM_OUTPUT_ROOT}/runs/robocoliseum_spatial
TENSORBOARD_LOG_DIR=${RUN_DIR}/logs
CACHE_ROOT=${ROBOCOLISEUM_OUTPUT_ROOT}/.cache_train
LOG_ROOT=${ROBOCOLISEUM_OUTPUT_ROOT}/logs/train_spatial
...
if [[ ! -d "${PROCESSED_SUITE_ROOT}" ]]; then
    echo "Preprocessing spatial datasets into offline 17-D format."
    "${PYTHON_BIN}" -u "${PREPROCESS_SCRIPT}"
fi
...
echo "purpose=RoboColiseum spatial instruction-following offline-17D/effective-16D post-training"
echo "processed_data=${PROCESSED_SUITE_ROOT}"
