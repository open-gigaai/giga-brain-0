...
CONFIG_RELATIVE_PATH=configs/robocoliseum_instruction_robust.py
CONFIG_PATH=${EXPERIMENT_ROOT}/${CONFIG_RELATIVE_PATH}
NORM_STATS_PATH=${ROBOCOLISEUM_NORM_STATS_ROOT}/robocoliseum_instruction_robust_17d.json
MODEL_DIR=${GIGABRAIN_PRETRAINED_CKPT}
MODEL_CONFIG=${MODEL_DIR}/config.json
PYTHON_BIN=${PYTHON_BIN:-python}
RUN_DIR=${ROBOCOLISEUM_OUTPUT_ROOT}/runs/robocoliseum_instruction_robust
TENSORBOARD_LOG_DIR=${RUN_DIR}/logs
CACHE_ROOT=${ROBOCOLISEUM_OUTPUT_ROOT}/.cache_train
LOG_ROOT=${ROBOCOLISEUM_OUTPUT_ROOT}/logs/train_instruction_robust
...
# Refuse to mix a fresh run with artifacts from an earlier launch.
if [[ -e "${RUN_DIR}/config.json" || -d "${RUN_DIR}/models" ]]; then
    echo "Fresh run directory already contains training artifacts: ${RUN_DIR}" >&2
    exit 1
fi
...
# Start with fresh optimizer and scheduler state from the selected checkpoint.
...
echo "purpose=RoboColiseum instruction_and_robust unified-17D/effective-16D post-training"
echo "reference=GigaBrain-0.7 open-source training stack"
echo "sampling=shape:3,common_sense:2,straighten:2,other_tasks:1_each"
echo "schedule=warmup_steps:1000,decay_steps:20000,max_steps:50000"
