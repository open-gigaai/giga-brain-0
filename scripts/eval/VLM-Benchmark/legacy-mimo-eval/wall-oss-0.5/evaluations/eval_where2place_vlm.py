#!/usr/bin/env python3
"""Evaluate Wall-OSS-0.5 VLM point prediction on Where2Place.

Where2Place provides an RGB image, a placement question, and a binary-ish mask
for the valid free-space region. The evaluator prompts the model to output one
or more pixel coordinates and scores whether predicted points fall inside the
mask.
"""
...
DEFAULT_DATASET_ROOT = (
    "datasets/public_datasets/VLM/vqa/benchmarks/Where2Place"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Where2Place root containing data/test-*.parquet.",
    )
    parser.add_argument(
        "--output-jsonl",
        default="where2place_wall_oss_0_5_predictions.jsonl",
    ...
    parser.add_argument(
        "--prompt-style",
        choices=["point-tags", "diverse-point-tags", "plain"],
        default="point-tags",
        help=(
            "point-tags asks for <point>[x, y]</point> outputs; "
            "diverse-point-tags asks for exactly 5 separated candidate points."
        ),
    )
    parser.add_argument(
        "--acc-k",
...
def build_prompt(question: str, image_size: tuple[int, int], prompt_style: str) -> str:
    width, height = image_size
    question = question.strip()
    if prompt_style == "plain":
        prompt_text = question
    elif prompt_style == "diverse-point-tags":
        prompt_text = (
            f"{question}\n"
            f"The image size is {width}x{height} pixels. "
            "Find the empty valid placement region described by the question. "
            "Return exactly 5 different candidate placement points that are all "
            "inside that empty region. Spread the points across the region when "
            "possible instead of repeating nearby coordinates. Use original "
            "image pixel coordinates, with x from left to right and y from top "
            "to bottom. Answer only with five point tags, for example "
            "<point>[x1, y1]</point><point>[x2, y2]</point><point>[x3, y3]</point>"
            "<point>[x4, y4]</point><point>[x5, y5]</point>."
        )
    else:
        prompt_text = (
            f"{question}\n"
            f"The image size is {width}x{height} pixels. "
            "Return 1 to 5 suitable placement points in original image pixel "
            "coordinates. Use x for horizontal coordinate from left to right and "
            "y for vertical coordinate from top to bottom. "
...
def extract_points(text: str) -> list[tuple[int, int]]:
    ...
    tag_pattern = re.compile(r"<(?:point|points)>\s*\[?([^\]<>]+)\]?\s*</(?:point|points)>", re.I)
    for match in tag_pattern.finditer(cleaned):
        nums = re.findall(r"-?\d+(?:\.\d+)?", match.group(1))
        for i in range(0, len(nums) - 1, 2):
            points.append((round(float(nums[i])), round(float(nums[i + 1]))))
    ...
    # Last-resort parser for outputs such as "x=120, y=240".
...
def iter_where2place(dataset_root: Path, mask_threshold: int) -> Iterable[dict]:
    ...
    columns = ["question_id", "question", "question_type", "image", "mask"]
...
def load_image(image_obj: dict, image_cls):
    ...
    if image_obj.get("bytes") is None:
        raise ValueError("Where2Place image field does not contain bytes")
    return image_cls.open(io.BytesIO(image_obj["bytes"]))


class Where2PlaceEvaluator:
    ...
    def predict_batch(self, samples: list[dict], prompt_style: str) -> list[dict]:
        ...
        prompts = [
            build_prompt(s["question"], s["image_size"], prompt_style) for s in samples
        ]
...
def main() -> int:
    ...
    evaluator = Where2PlaceEvaluator(args)
    ...
    def filtered_samples():
        seen = 0
        for sample in iter_where2place(dataset_root, args.mask_threshold):
    ...
    with output_path.open("w", encoding="utf-8") as f:
        for batch in batched(filtered_samples(), args.batch_size):
            ...
            for sample, pred in zip(batch, predictions):
                ...
                stats = by_type[sample["question_type"]]
                stats["total"] += 1
                ...
                record = {
                    ...
                    "sample_hit": has_hit,
                    **{f"acc_at_{k}": hit for k, hit in sample_acc_at_k.items()},
    ...
    summary = {
        ...
        **{f"point_acc_at_{k}": acc_counts[k] / total if total else 0.0 for k in acc_ks},
        ...
        "point_hit_rate": hit_points / total_points if total_points else 0.0,
