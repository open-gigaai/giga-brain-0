#!/usr/bin/env python3
"""Evaluate Wall-OSS-0.5 VLM point grounding on RoboAfford-eval.

RoboAfford-eval contains one image, one point-localization question, and one
target mask per sample. The evaluator prompts the model to output points and
scores whether predicted points fall inside the mask.
"""
...
DEFAULT_DATASET_ROOT = "datasets/public_datasets/VLM/vqa/roboafford-eval"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="RoboAfford-eval root containing data/test-*.parquet.",
    )
    parser.add_argument(
        "--output-jsonl",
        default="roboafford_eval_wall_oss_0_5_predictions.jsonl",
    ...
    parser.add_argument(
        "--prompt-style",
        choices=["dataset", "normalized-point-tags", "pixel-point-tags", "plain"],
        default="dataset",
        help=(
            "dataset preserves the dataset question; normalized-point-tags asks "
            "for 0-1 coordinates; pixel-point-tags asks for image pixel coordinates."
        ),
    )
    parser.add_argument(
        "--coord-mode",
...
def build_prompt(question: str, image_size: tuple[int, int], prompt_style: str) -> str:
    ...
    if prompt_style == "plain":
        ...
        prompt_text = (
            f"{question}\n"
            "Return 1 to 5 points that satisfy the question. Use normalized "
            "coordinates between 0 and 1, where x is horizontal from left to "
            "right and y is vertical from top to bottom. Answer only as "
            "<answer><point>[(x1, y1), (x2, y2), ...]</point></answer>."
        )
    elif prompt_style == "pixel-point-tags":
        prompt_text = (
            f"{question}\n"
            f"The image size is {width}x{height} pixels. Return 1 to 5 points "
            "that satisfy the question in original image pixel coordinates. "
            "Use x for horizontal coordinate from left to right and y for "
            "vertical coordinate from top to bottom. Answer only as "
            "<answer><point>[[x1, y1], [x2, y2], ...]</point></answer>."
        )
    else:
        prompt_text = question
...
def iter_roboafford_eval(dataset_root: Path, mask_threshold: int) -> Iterable[dict]:
    ...
    columns = ["image", "mask", "question", "category"]
    row_index = 0
...
def load_image(image_obj: dict, image_cls):
    ...
    if image_obj.get("bytes") is None:
        raise ValueError("RoboAfford-eval image field does not contain bytes")
    return image_cls.open(io.BytesIO(image_obj["bytes"]))


class RoboAffordEvalEvaluator:
...
def main() -> int:
    ...
    evaluator = RoboAffordEvalEvaluator(args)
    ...
    def filtered_samples():
        seen = 0
        for sample in iter_roboafford_eval(dataset_root, args.mask_threshold):
