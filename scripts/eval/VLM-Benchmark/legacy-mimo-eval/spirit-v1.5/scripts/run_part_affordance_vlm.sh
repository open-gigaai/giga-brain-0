...
DATASET_PATH="${DATASET_PATH:-datasets/public_datasets/VLM/vqa/Part-Affordance-2K}"
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/part_affordance_vlm}"
...
if [[ ! -d "${DATASET_PATH}/data" ]]; then
  echo "[ERROR] Part-Affordance data directory not found: ${DATASET_PATH}/data" >&2
  exit 1
fi
...
"${PYTHON_BIN}" -m evaluations.part_affordance_vlm \
  --ckpt-path "${CKPT_PATH}" \
