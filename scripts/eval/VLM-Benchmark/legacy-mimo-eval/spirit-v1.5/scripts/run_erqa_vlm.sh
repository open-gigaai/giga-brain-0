...
DATASET_PATH="${DATASET_PATH:-datasets/public_datasets/VLM/vqa/benchmarks/ERQA}"
...
if [[ ! -d "${DATASET_PATH}/data" ]]; then
  echo "[ERROR] ERQA data directory not found: ${DATASET_PATH}/data" >&2
  exit 1
fi
...
echo "[INFO] MAX_IMAGES_PER_SAMPLE=${MAX_IMAGES_PER_SAMPLE}"
...
"${PYTHON_BIN}" -m evaluations.erqa_vlm \
  --ckpt-path "${CKPT_PATH}" \
