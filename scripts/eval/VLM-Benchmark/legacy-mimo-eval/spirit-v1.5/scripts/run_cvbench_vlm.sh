...
DATASET_PATH="${DATASET_PATH:-datasets/public_datasets/VLM/vqa/CV-Bench}"
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/cvbench_vlm}"
...
if [[ ! -f "${DATASET_PATH}/test_2d.parquet" ]]; then
  echo "[ERROR] CV-Bench test_2d.parquet not found: ${DATASET_PATH}/test_2d.parquet" >&2
  exit 1
fi
if [[ ! -f "${DATASET_PATH}/test_3d.parquet" ]]; then
  echo "[ERROR] CV-Bench test_3d.parquet not found: ${DATASET_PATH}/test_3d.parquet" >&2
  exit 1
fi
...
"${PYTHON_BIN}" -m evaluations.cvbench_vlm \
  --ckpt-path "${CKPT_PATH}" \
