...
DATASET_PATH="${DATASET_PATH:-datasets/public_datasets/VLM/vqa/roboafford-eval}"
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/roboafford_vlm}"
...
if [[ ! -d "${DATASET_PATH}/data" ]]; then
  echo "[ERROR] RoboAfford data directory not found: ${DATASET_PATH}/data" >&2
  exit 1
fi
...
"${PYTHON_BIN}" -m evaluations.roboafford_vlm \
  --ckpt-path "${CKPT_PATH}" \
