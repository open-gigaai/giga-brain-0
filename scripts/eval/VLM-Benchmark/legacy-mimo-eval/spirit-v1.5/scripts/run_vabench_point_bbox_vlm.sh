...
DATASET_PATH="${DATASET_PATH:-datasets/public_datasets/VLM/benchmarks/vabench-point-bbox/llava_json/vabench_point_bbox_llava.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/vabench_point_bbox_vlm}"
...
if [[ ! -f "${DATASET_PATH}" && ! -d "${DATASET_PATH}/data" ]]; then
  echo "[ERROR] VABench point-bbox dataset not found. Expected a LLaVA jsonl file or parquet data directory: ${DATASET_PATH}" >&2
  exit 1
fi
...
"${PYTHON_BIN}" -m evaluations.vabench_point_bbox_vlm \
  --ckpt-path "${CKPT_PATH}" \
