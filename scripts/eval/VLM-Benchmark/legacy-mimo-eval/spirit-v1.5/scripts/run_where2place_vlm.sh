...
DATASET_PATH="${DATASET_PATH:-datasets/public_datasets/VLM/vqa/benchmarks/Where2Place}"
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/where2place_vlm}"
...
if [[ ! -d "${DATASET_PATH}/data" ]]; then
  echo "[ERROR] Where2Place data directory not found: ${DATASET_PATH}/data" >&2
  exit 1
fi
...
"${PYTHON_BIN}" -m evaluations.where2place_vlm \
  --ckpt-path "${CKPT_PATH}" \
