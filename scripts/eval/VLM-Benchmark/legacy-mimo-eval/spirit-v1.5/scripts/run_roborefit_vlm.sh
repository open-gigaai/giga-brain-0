...
DATASET_PATH="${DATASET_PATH:-datasets/public_datasets/VLM/benchmarks/roborefit/llava_json/roborefit_test_llava.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/roborefit_vlm}"
...
if [[ ! -f "${DATASET_PATH}" && ! -d "${DATASET_PATH}/data" ]]; then
  echo "[ERROR] RoboRefIt dataset not found. Expected a LLaVA jsonl file or parquet data directory: ${DATASET_PATH}" >&2
  exit 1
fi
...
"${PYTHON_BIN}" -m evaluations.roborefit_vlm \
  --ckpt-path "${CKPT_PATH}" \
