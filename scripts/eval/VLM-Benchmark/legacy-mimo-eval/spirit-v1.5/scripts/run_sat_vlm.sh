...
DATASET_PATH="${DATASET_PATH:-datasets/public_datasets/VLM/vqa/benchmarks/SAT}"
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/outputs/sat_vlm}"
...
if [[ ! -f "${DATASET_PATH}/SAT_test.parquet" ]]; then
  echo "[ERROR] SAT_test.parquet not found: ${DATASET_PATH}/SAT_test.parquet" >&2
  exit 1
fi
...
"${PYTHON_BIN}" -m evaluations.sat_vlm \
  --ckpt-path "${CKPT_PATH}" \
