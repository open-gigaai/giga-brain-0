...
DEFAULT_DATASET_PATH = "datasets/public_datasets/VLM/vqa/Part-Affordance-2K"
DEFAULT_SUFFIX = (
    "Return 3 candidate grasp points on the object part that affords the requested action. "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate the Spirit-v1.5 Qwen3-VL backbone on Part-Affordance-2K.",
    )
    parser.add_argument("--ckpt-path", default=DEFAULT_CKPT_PATH, help="Spirit-v1.5 checkpoint directory.")
    parser.add_argument("--dataset-path", default=DEFAULT_DATASET_PATH, help="Part-Affordance-2K dataset directory.")
    parser.add_argument(
        "--processor-path",
        default=None,
    ...
    parser.add_argument("--output-dir", default="outputs/part_affordance_vlm", help="Directory for outputs.")
    parser.add_argument("--split", default="train", help="Dataset split name when loading through datasets.")
    ...
    parser.add_argument(
        "--system-prompt",
        default="You are a helpful robotic affordance grounding assistant.",
        help="System prompt used for coordinate generation.",
    )
    parser.add_argument(
        "--question-suffix",
        default=DEFAULT_SUFFIX,
        help="Instruction appended to each Part-Affordance problem.",
    )
    parser.add_argument("--mask-threshold", type=int, default=127, help="Mask threshold for valid affordance region.")
    parser.add_argument(
        "--coordinate-scale",
...
def decode_image(image_obj: Any, mode: str) -> Image.Image:
    ...
    raise TypeError(f"Unsupported Part-Affordance image object: {type(image_obj)!r}")


def load_part_affordance(dataset_path: Path, split: str, max_samples: int | None) -> list[PartAffordanceSample]:
    try:
        from datasets import load_dataset
    except ImportError as exc:
        raise ImportError(
            "Part-Affordance-2K is stored as parquet with image columns. Install datasets/pyarrow "
    ...
    samples: list[PartAffordanceSample] = []
    for idx, row in enumerate(dataset):
        samples.append(
            PartAffordanceSample(
                sample_id=idx,
                question_id=int(row["question_id"]),
                problem=str(row["problem"]).strip(),
                category_type=str(row["category_type"]).strip(),
...
def chunked(items: list[PartAffordanceSample], size: int) -> Iterable[list[PartAffordanceSample]]:
    for start in range(0, len(items), size):
        yield items[start : start + size]


def build_messages(sample: PartAffordanceSample, system_prompt: str, question_suffix: str) -> list[dict[str, Any]]:
...
def parse_points(text: str, width: int, height: int, coordinate_scale: str) -> list[tuple[int, int]]:
    point_section = re.search(r"<point>(.*?)</point>", text, flags=re.IGNORECASE | re.DOTALL)
    source = point_section.group(1) if point_section else text
    number = r"-?\d+(?:\.\d+)?"
...
def score_points(points: list[tuple[int, int]], mask: Image.Image, threshold: int) -> tuple[int, int, float, bool]:
    ...
    total = len(points)
    return hits, total, hits / max(total, 1), hits > 0
...
def build_category_metrics(records: list[dict[str, Any]]) -> dict[str, dict[str, float | int]]:
    metrics: dict[str, dict[str, float | int]] = {}
    for record in records:
        key = record["category_type"]
...
def main() -> None:
    ...
    samples = load_part_affordance(dataset_path, args.split, args.max_samples)
    ...
    print(
        f"Loaded {len(samples)} Part-Affordance samples; evaluating {len(samples_to_run)} "
    ...
    summary = {
        ...
        "by_category_type": build_category_metrics(all_records),
