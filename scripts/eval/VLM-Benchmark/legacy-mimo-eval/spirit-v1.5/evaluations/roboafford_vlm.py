...
DEFAULT_DATASET_PATH = "datasets/public_datasets/VLM/vqa/roboafford-eval"


@dataclass
class RoboAffordSample:
    sample_id: int
    question: str
    category: str
    image: Image.Image
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate the Spirit-v1.5 Qwen3-VL backbone on RoboAfford-Eval.",
    )
    parser.add_argument("--ckpt-path", default=DEFAULT_CKPT_PATH, help="Spirit-v1.5 checkpoint directory.")
    parser.add_argument("--dataset-path", default=DEFAULT_DATASET_PATH, help="RoboAfford-Eval dataset directory.")
    parser.add_argument(
        "--processor-path",
        default=None,
    ...
    parser.add_argument("--output-dir", default="outputs/roboafford_vlm", help="Directory for outputs.")
    ...
    parser.add_argument("--question-suffix", default="", help="Optional suffix appended to each question.")
...
def decode_image(image_obj: Any, mode: str) -> Image.Image:
    ...
    raise TypeError(f"Unsupported RoboAfford image object: {type(image_obj)!r}")


def load_roboafford(dataset_path: Path, split: str, max_samples: int | None) -> list[RoboAffordSample]:
    try:
        from datasets import load_dataset
    except ImportError as exc:
        raise ImportError(
            "RoboAfford-Eval is stored as parquet with image columns. Install datasets/pyarrow "
...
def chunked(items: list[RoboAffordSample], size: int) -> Iterable[list[RoboAffordSample]]:
    for start in range(0, len(items), size):
        yield items[start : start + size]


def build_messages(sample: RoboAffordSample, system_prompt: str, question_suffix: str) -> list[dict[str, Any]]:
...
def main() -> None:
    ...
    samples = load_roboafford(dataset_path, args.split, args.max_samples)
    ...
    print(
        f"Loaded {len(samples)} RoboAfford samples; evaluating {len(samples_to_run)} "
    ...
    summary = {
        ...
        "by_category": build_category_metrics(all_records),
