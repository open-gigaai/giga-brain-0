...
DEFAULT_DATA_PATH = (
    "datasets/public_datasets/VLM/benchmarks/RealWorldQA/llava_json/"
    "realworldqa_test_llava.jsonl"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate XR-0 on RealWorldQA LLaVA-format JSONL.")
    parser.add_argument("--data-path", default=DEFAULT_DATA_PATH, help="Path to realworldqa_test_llava.jsonl.")
    parser.add_argument(
        "--image-root",
        default=None,
        help="Optional root used when image paths in the JSONL are relative.",
    )
    add_model_args(parser, "eval_vlm/results/realworldqa_llava_predictions.jsonl")
    parser.set_defaults(model_path=DEFAULT_MODEL_PATH, max_new_tokens=16)
...
def image_path_from_row(row: Dict[str, Any], data_path: Path, image_root: Optional[Path]) -> Path:
    image_field = row.get("image")
    if isinstance(image_field, list):
        if len(image_field) != 1:
            raise ValueError(f"Expected one image per RealWorldQA sample, got {len(image_field)}")
...
def read_records(data_path: Path, image_root: Optional[Path], limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    with data_path.open("r", encoding="utf-8") as f:
        for line_idx, line in enumerate(f):
            ...
            answer = conversation_value(row, "gpt").strip()
            image_path = image_path_from_row(row, data_path, image_root)
            records.append(
                {
                    ...
                    "prompt": build_vlm_prompt(
                        question,
                        answer_instruction="Answer directly with the shortest correct answer.",
                    ),
                }
            )
...
def score_prediction(prediction: str, answer: str) -> bool:
    ...
    if re.fullmatch(r"[A-Z]", answer, flags=re.IGNORECASE):
        return first_letter(prediction) == answer.upper()
...
def main() -> None:
    ...
    print(f"Loaded {len(records)} RealWorldQA LLaVA examples from {data_path}")

    model, processor, device = load_model_and_processor(args)
    outputs = generate_predictions(model, processor, device, records, args, "RealWorldQA-LLaVA")
