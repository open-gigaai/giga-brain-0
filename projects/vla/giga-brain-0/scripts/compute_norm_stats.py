...
from giga_brain_0.giga_brain_0_transforms import robot_type_mapping as shared_robot_type_mapping
...
class GetEmbodimentId:
    """Resolve embodiment id using the shared robot_type mapping."""

    valid_embodiment_ids = {
        int(embodiment_id): embodiment_id.name.lower()
        for embodiment_id in sorted(set(shared_robot_type_mapping.values()), key=int)
    }

    robot_type_mapping = {
        str(robot_type): int(embodiment_id)
        for robot_type, embodiment_id in shared_robot_type_mapping.items()
    }

    def __call__(self, data: dict[str, Any]) -> dict[str, Any]:
        if data.get('embodiment_id') is not None:
            embodiment_id = int(data['embodiment_id'])
        else:
            robot_type = data['meta'].info['robot_type']
            embodiment_id = self.robot_type_mapping[str(robot_type)]
...
def compute_norm_stats(
    ...
    data_or_config = [
        dict(
            ...
            delta_info=dict(
                # action=action_chunk,
                **({'action_tcp_endpose_quat': action_chunk} if Embodiment3QuaternionTo6D.supports_embodiment(embodiment_id) else {'action': action_chunk}),
            ),
            meta_name='meta',
